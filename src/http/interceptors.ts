...
export function requestSuccessFunc(config: AxiosRequestConfig) {
    // 自定义请求拦截逻辑，可以处理权限，请求发送监控等
    config.headers['Accept-Language'] = GlobalModule.language;
    return config;
}
...
export function responseSuccessFunc(responseObj: AxiosResponse) {
    ...
    switch (code) {
        ...
        default:
            ...
            if (responseObj.config.customConfig && !responseObj.config.customConfig.noShowDefaultError) {
                window.$vbus.$emit('global.$dialog.show', resData.msg);
            }
