...
export const getPlatform: () => PlatformInfo = () => {
  ...
  const platformInfo: PlatformInfo = { platform: 'browser' };
  if (info && info.length > 0) {
    const infoArray = info[0].split('_');
    platformInfo.platform = infoArray[3] as Platform;
    platformInfo.appVersion = infoArray[1];
    platformInfo.systemVersion = infoArray[2];
  }
  return platformInfo;
};
